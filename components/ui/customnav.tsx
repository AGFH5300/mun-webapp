...
const CustomNav: React.FC<CustomNavProps> = ({ embedded = false }) => {
  ...
  useEffect(() => {
    ...
    const syncCounts = () => {
      const unread = Number(window.localStorage.getItem("mun.messages.unreadTotal") || "0");
      const notifications = Number(window.localStorage.getItem("mun.messages.notificationTotal") || "0");
    ...
    const seen = Number(window.localStorage.getItem("mun.messages.seenNotificationTotal") || "0");
    setSeenNotificationCount(Number.isFinite(seen) ? Math.max(0, seen) : 0);
    syncCounts();
    const onUnread = () => syncCounts();
    const onNotifications = () => syncCounts();
    window.addEventListener("mun:messages-unread-updated", onUnread as EventListener);
    window.addEventListener("mun:messages-notification-updated", onNotifications as EventListener);
    window.addEventListener("storage", syncCounts);
    return () => {
      window.removeEventListener("mun:messages-unread-updated", onUnread as EventListener);
      window.removeEventListener("mun:messages-notification-updated", onNotifications as EventListener);
  ...
  const markNotificationsSeen = () => {
    ...
    window.localStorage.setItem("mun.messages.seenNotificationTotal", String(nextSeenCount));
  };
